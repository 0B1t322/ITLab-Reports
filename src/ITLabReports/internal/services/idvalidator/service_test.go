	...
	t.Run(
		...
		func(t *testing.T) {
			...
			require.Condition(
				t,
				func() (success bool) {
					return errors.Is(err, idvalidator.ErrFailedToValidateId) &&
						errors.Unwrap(err).Error() == "Some error"
				},
			)
		},
	)
	...
	t.Run(
		...
		func(t *testing.T) {
			...
			require.Condition(
				t,
				func() (success bool) {
					return errors.Is(err, idvalidator.ErrIdInvalid) &&
						errors.Unwrap(err).Error() == "invalid_id"
				},
			)
		},
	)
